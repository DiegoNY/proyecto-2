...
/**
 * 
 * si los datos son vacios no deja ingresar datos peros si si contiene datos se ingresan a la bd 
 * 
 */
function validandoDatos($direccion, $celular, $nombre, $codUbigeo, $eliminada, $razonSocial, $pais, $ruc, $trabaja, $bd)
{
    if (empty($direccion) || empty($celular) || empty($nombre) || empty($codUbigeo) || empty($razonSocial) || empty($pais)) {
        ...
        if ($resultado == TRUE) {
            $volver = header("Location: pantallaRegistrarSucursal.php?mensaje=registrada&&ruc=$ruc");
        } else {
